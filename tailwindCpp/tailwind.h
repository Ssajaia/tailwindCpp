...
namespace tw {
    ...
    public:
        static std::string render(const std::string& text, const StyleState& style) {
            ...
            // 5. Apply margin
            std::string margined = apply_margin(aligned, style);

            // 6. Apply ANSI styling (always last)
            result = apply_ansi_styling(margined, style);
    ...
    private:
        ...
        static std::string apply_border(const std::string& text, const StyleState& style) {
            if (style.border == BorderType::None) return text;
            ...
            for (size_t i = 0; i < content_width + 2; ++i) {
                result += border_chars.horizontal;
            }
        ...
        static std::string apply_alignment(const std::string& text, const StyleState& style) {
            if (style.alignment == Alignment::Left) return text;
        ...
        static std::string apply_ansi_styling(const std::string& text, const StyleState& style) {
            ...
            // Modifiers
            if (style.has_modifier(Bold)) { result += "1;"; has_styling = true; }
            if (style.has_modifier(Dim)) { result += "2;"; has_styling = true; }
            if (style.has_modifier(Underline)) { result += "4;"; has_styling = true; }
            if (style.has_modifier(Blink)) { result += "5;"; has_styling = true; }
            if (style.has_modifier(Reverse)) { result += "7;"; has_styling = true; }
            if (style.has_modifier(Hidden)) { result += "8;"; has_styling = true; }
            ...
            if (has_styling && !result.empty() && result.back() == ';') {
                result.pop_back();
            }
    ...
    public:
        ...
        Text& tw(const std::string& utility_str) {
            style = Parser::parse(utility_str);
            return *this;
        }
